...
def str2type(v):
    if v.lower() == "fp32" or v.lower() == "fp16":
        ...
        raise argparse.ArgumentTypeError(
            "Invalid type, currently supported type: [fp32, fp16]"
        )
...
def scheduler_type_choice(v):
    if v == "annealing" or v == "custom_warmup_anneal":
        ...
        raise argparse.ArgumentTypeError(
            "Argument must be either " "'annealing', " "'custom_warmup_anneal'"
        )
...
def optim_type_choice(v):
    if v == "adam" or v == "radam":
        ...
        raise argparse.ArgumentTypeError("Argument must be either 'adam', " "'radam'.")
