...
def load_most_recent_checkpoint(
    model,
    optimizer=None,
    scheduler=None,
    data_loader=None,
    ...
    for file_name in ls_files:
        if file_name.startswith("checkpoint_"):
            ...
            if (most_recent_checkpoint_datetime is None) or (
                most_recent_checkpoint_datetime is not None
                and file_datetime > most_recent_checkpoint_datetime
            ):
    ...
    if most_recent_checkpoint_filename is not None:
        ...
        checkpoint = torch.load(
            save_model_path + most_recent_checkpoint_filename, map_location=map_location
        )
...
def save_last_checkpoint(
    ...
    for file_name in ls_files:
        if file_name.startswith("checkpoint_"):
            ...
            if (oldest_checkpoint_datetime is None) or (
                oldest_checkpoint_datetime is not None
                and file_datetime < oldest_checkpoint_datetime
            ):
                oldest_checkpoint_datetime = file_datetime
                oldest_checkpoint_filename = file_name

    if (
        oldest_checkpoint_filename is not None
        and num_check_points == num_max_checkpoints
    ):
    ...
    new_checkpoint_filename = (
        ...
        + "_epoch"
        + str(data_loader.get_epoch_it())
        + "it"
        + str(data_loader.get_batch_it())
        + "bs"
