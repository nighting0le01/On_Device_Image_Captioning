...
from legacy_models.layers import EmbeddingLayer, DecoderLayer, EncoderLayer
...
class End_ExpansionNet_v2(CaptioningModel):
    ...
    def forward_enc(self, enc_input, enc_input_num_pads):
        assert enc_input_num_pads is None or enc_input_num_pads == (
            [0] * enc_input.size(0)
        ...
        pos_x = (
            ...
            .unsqueeze(0)
            .expand(enc_input.size(0), max_num_enc)
            .to(self.rank)
        )
    ...
    def forward_dec(
        ...
        pos_x = (
            ...
            .unsqueeze(0)
            .expand(dec_input.size(0), self.num_exp_dec)
            .to(self.rank)
        )
        pos_y = (
            torch.arange(dec_input.size(1))
            .unsqueeze(0)
            .expand(dec_input.size(0), dec_input.size(1))
            .to(self.rank)
        )
