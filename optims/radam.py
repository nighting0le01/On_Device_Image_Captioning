        ...
        for group in self.param_groups:
            for p in group["params"]:
                ...
                exp_avg_sq.mul_(beta2).addcmul_(
                    value=1 - beta2, tensor1=grad, tensor2=grad
                )
                ...
                if N_sma >= 5:
                    ...
                    p_data_fp32.addcdiv_(
                        value=-step_size * group["lr"], tensor1=exp_avg, tensor2=denom
                    )
        ...
        for group in self.param_groups:
            for p in group["params"]:
                ...
                if N_sma >= 5:
                    ...
                    denom = exp_avg_sq.sqrt().add_(group["eps"])
                    p_data_fp32.addcdiv_(
                        value=-step_size, tensor1=exp_avg, tensor2=denom
                    )
                    p.data.copy_(p_data_fp32)
                elif self.degenerated_to_sgd:
                    if group["weight_decay"] != 0:
    ...
    def step(self, closure=None):
        ...
        for group in self.param_groups:
            for p in group["params"]:
                ...
                if group["weight_decay"] != 0:
                    p_data_fp32.add_(
                        alpha=-group["weight_decay"] * scheduled_lr, other=p_data_fp32
                    )
