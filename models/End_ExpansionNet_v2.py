...
class End_ExpansionNet_v2(CaptioningModel):
    ...
    def forward_enc(self, enc_input, enc_input_num_pads):
        ...
        pos_x = torch.arange(max_num_enc).to(self.rank).to(dtype=torch.float32).unsqueeze(0).repeat_interleave(enc_input.size(0), dim=0).to(self.rank)
        pad_mask = create_pad_mask(mask_size=(enc_input.size(0), max_num_enc, enc_input.size(0)),
        ...
        for i in range(self.N_enc):
            x = self.encoders[i](x=x, n_indexes=pos_x.to(dtype=torch.int32), mask=pad_mask)
            x_list.append(x)
    ...
    def forward_dec(self, cross_input, enc_input_num_pads, dec_input, dec_input_num_pads):
        ...
        pos_x = torch.arange(self.num_exp_dec).to(self.rank).to(dtype=torch.float32).unsqueeze(0).repeat_interleave(dec_input.size(0), dim=0).to(self.rank)
        range_dec = torch.tensor([1.]).to(self.rank).repeat_interleave(dec_input.size(1), dim=0).cumsum(dim=-1) - 1
...
class E2E_ExpansionNet_Captioner(Captioner):
    ...
    def __init__(self, beam_search_args, model=None, split_encoder = False, apply_log_softmax = False, 
                    encoder=None, decoder=None, rank = 0, N_enc=3, N_dec=3, num_exp_dec=16, 
                    num_exp_enc_list=[32, 64, 128, 256, 512]):
        super().__init__(beam_search_args, model, split_encoder, apply_log_softmax,  encoder, decoder)
        self.rank = rank
        self.N_enc = N_enc
        self.N_dec = N_dec
        self.num_exp_dec = num_exp_dec
    ...
    def forward_dec(self, cross_input, enc_input_num_pads, dec_input, dec_input_num_pads):
        if self.split_encoder: 
            y = self.decoder(cross_input,  dec_input, enc_input_num_pads, dec_input_num_pads)     
            if self.apply_log_softmax:
                y = self.log_softmax(y)
            return y
        else: 

            return self.model.forward_dec(cross_input, enc_input_num_pads, dec_input, dec_input_num_pads)
...
class End_ExpansionNet_v2_Encoder(CaptioningModel):
    def __init__(self,
        ...
        self.swin_transf = SwinTransformer(
                 ...
                 drop_rate=swin_drop_rate, attn_drop_rate=swin_attn_drop_rate, drop_path_rate=swin_drop_path_rate,
                 norm_layer=swin_norm_layer, ape=swin_ape, patch_norm=swin_patch_norm,
                 use_checkpoint=swin_use_checkpoint, rank=self.rank)
    ...
    def forward_enc(self, enc_input, enc_input_num_pads):
        ...
        enc_input_num_pads = [0] * enc_input.size(0)
        max_num_enc = sum(self.num_exp_enc_list)
        pos_x = torch.arange(max_num_enc).to(self.rank).to(dtype=torch.float32).unsqueeze(0).repeat_interleave(enc_input.size(0), dim=0)
        pad_mask = create_pad_mask(mask_size=(enc_input.size(0), max_num_enc, enc_input.size(0)),
                                   pad_row=[0] * enc_input.size(0),
                                   pad_column=enc_input_num_pads,
                                   rank=self.rank)
...
class End_ExpansionNet_v2_Decoder(CaptioningModel):
    ...
    def forward_dec(self, cross_input, enc_input_num_pads, dec_input, dec_input_num_pads):
        ...
        pos_x = torch.arange(self.num_exp_dec).to(self.rank).to(dtype=torch.float32).unsqueeze(0).repeat_interleave(dec_input.size(0), dim=0)
        range_dec = torch.tensor([1.]).to(self.rank).repeat_interleave(dec_input.size(1), dim=0).cumsum(dim=-1) - 1
        pos_y = range_dec.unsqueeze(0).repeat_interleave(dec_input.size(0), dim=0)
        y = y + self.pos_encoder(pos_y.to(dtype=torch.int32))
        y_list = []
