...
from models.captioning_model import CaptioningModel, Captioner
...
class End_ExpansionNet_v2(CaptioningModel):
    def __init__(
        ...
        super(End_ExpansionNet_v2, self).__init__(apply_log_softmax)
        ...
        self.out_embedder = EmbeddingLayer(
            len(output_word2idx), d_model, drop_args.dec_input
        )
    ...
    def forward_enc(self, enc_input, enc_input_num_pads):
        ...
        for i in range(self.N_enc):
            x = self.encoders[i](
                x=x, n_indexes=pos_x.to(dtype=torch.int32), mask=pad_mask
            )
            x_list.append(x)
    ...
    def forward_dec(
        ...
        pad_mask = create_pad_mask(
            mask_size=(dec_input.size(0), dec_input.size(1), cross_input.size(1)),
        ...
        range_dec = (
            torch.tensor([1.0])
            .to(self.rank)
            .repeat_interleave(dec_input.size(1), dim=0)
            .cumsum(dim=-1)
            - 1
        )
        pos_y = range_dec.unsqueeze(0).repeat_interleave(dec_input.size(0), dim=0)
        y = y + self.pos_encoder(pos_y.to(dtype=torch.int32))
        y_list = []
        for i in range(self.N_dec):
            y = self.decoders[i](
                x=y,
                n_indexes=pos_x.to(dtype=torch.int32),
...
class E2E_ExpansionNet_Captioner(Captioner):
    def __init__(
        self,
        beam_search_args,
        model=None,
    ...
    def forward_dec(
        ...
        if self.split_encoder:
            y = self.decoder(
                cross_input, dec_input, enc_input_num_pads, dec_input_num_pads
            )
            if self.apply_log_softmax:
                y = self.log_softmax(y)
            return y
...
class End_ExpansionNet_v2_Encoder(CaptioningModel):
    def __init__(
        self,
        # swin transf
        swin_img_size,
...
class End_ExpansionNet_v2_Decoder(CaptioningModel):
    def __init__(
        self,
        # captioning
        d_model,
