...
class Captioner:
    def __init__(self, beam_search_args, model=None, split_encoder = False, apply_log_softmax = False, 
      ...
      if self.split_encoder: 
        ...
        if self.encoder is None or self.decoder is None: 
          raise ValueError("Both encoder and decoder must be supplied in Split Encoder mode")
      else: 
        self.model = model
    ...
    def __call__(self, enc_x, dec_x=None, 
                  enc_x_num_pads=[0], dec_x_num_pads=[0], mode="beam_search"):
          assert ('sos_idx' in self.beam_search_args.keys() or 'eos_idx' in self.beam_search_args.keys()), \
