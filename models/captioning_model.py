...
@torch.fx.wrap
def loop_end_condition(loop_vector, time_step, bs, beam_size):
    return (loop_vector != time_step + 1).sum() == (bs * beam_size)


class CaptioningModel(nn.Module):
    def __init__(self, apply_log_softmax=False):
    ...
    def check_required_attributes(self):
        if self.rank is None:
            raise NotImplementedError(
                "Subclass must assign the rank integer according to the GPU group"
            )
    ...
    def forward_dec(
        self, cross_input, enc_input_num_pads, dec_input, dec_input_num_pads
    ):
        raise NotImplementedError

    def forward(self, enc_x, dec_x=None, enc_x_num_pads=[0], dec_x_num_pads=[0]):
...
class Captioner:
    def __init__(
        ...
        if self.split_encoder:
            ...
            if self.encoder is None or self.decoder is None:
                raise ValueError(
                    "Both encoder and decoder must be supplied in Split Encoder mode"
                )
        else:
            self.model = model
            if self.model is None:
                raise ValueError("An Encoder-Decoder model must be provided")
    ...
    def __call__(
        ...
        assert (
            "sos_idx" in self.beam_search_args.keys()
            or "eos_idx" in self.beam_search_args.keys()
        ), "sos and eos must be provided in case of batch sampling or beam search"
        sos_idx = self.beam_search_args["sos_idx"]
        eos_idx = self.beam_search_args["eos_idx"]
        if mode == "beam_search":
            self.apply_log_softmax = True
            beam_size_arg = self.beam_search_args.get("beam_size", 5)
            how_many_outputs_per_beam = self.beam_search_args.get("how_many_outputs", 1)
            beam_max_seq_len = self.beam_search_args.get("beam_max_seq_len", 20)
            sample_or_max = self.beam_search_args.get("sample_or_max", "max")
        ...
        if mode == "sampling":
            self.apply_log_softmax = True
            how_many_outputs = self.beam_search_args.get("how_many_outputs", 1)
            sample_max_seq_len = self.beam_search_args.get("sample_max_seq_len", 20)
    ...
    def get_batch_multiple_sampled_prediction(
        self, enc_input, enc_input_num_pads, num_outputs, sos_idx, eos_idx, max_seq_len
    ):
        bs, enc_seq_len, _ = enc_input.shape

        enc_input_num_pads = [
            enc_input_num_pads[i] for i in range(bs) for _ in range(num_outputs)
        ]
        ...
        x = (
            ...
            .reshape(bs * num_outputs, enc_seq_len, x.shape[-1])
        )

        upperbound_vector = torch.tensor(
            [max_seq_len] * bs * num_outputs, dtype=torch.int
        ).to(self.rank)
        where_is_eos_vector = upperbound_vector.clone()
        eos_vector = torch.tensor([eos_idx] * bs * num_outputs, dtype=torch.long).to(
            self.rank
        )
        finished_flag_vector = torch.zeros(bs * num_outputs).type(torch.int)

        predicted_caption = (
            torch.tensor([sos_idx] * (bs * num_outputs), dtype=torch.long)
            .to(self.rank)
            .unsqueeze(-1)
        )
        predicted_caption_prob = (
            torch.zeros(bs * num_outputs).to(self.rank).unsqueeze(-1)
        )

        dec_input_num_pads = [0] * (bs * num_outputs)
        time_step = 0
        while (
            finished_flag_vector.sum() != bs * num_outputs
        ) and time_step < max_seq_len:
            dec_input = predicted_caption
            log_probs = self.forward_dec(
                x, enc_input_num_pads, dec_input, dec_input_num_pads
            )

            prob_dist = torch.distributions.Categorical(
                torch.exp(log_probs[:, time_step])
            )
            sampled_word_indexes = prob_dist.sample()

            predicted_caption = torch.cat(
                (predicted_caption, sampled_word_indexes.unsqueeze(-1)), dim=-1
            )
            predicted_caption_prob = torch.cat(
                (
                    predicted_caption_prob,
                    log_probs[:, time_step].gather(
                        index=sampled_word_indexes.unsqueeze(-1), dim=-1
                    ),
                ),
                dim=-1,
            )
            ...
            where_is_eos_vector = torch.min(
                where_is_eos_vector,
                upperbound_vector.masked_fill(
                    sampled_word_indexes == eos_vector, time_step
                ),
            )
            finished_flag_vector = torch.max(
                finished_flag_vector,
        ...
        for i in range(bs):
            ...
            for j in range(num_outputs):
                ...
                res_predicted_caption[i].append(
                    predicted_caption[
                        index, : where_is_eos_vector[index].item() + 1
                    ].tolist()
                )

        where_is_eos_vector = where_is_eos_vector.unsqueeze(-1).expand(
            -1, time_step + 1
        )
        arange_tensor = (
            torch.arange(time_step + 1)
            .unsqueeze(0)
            .expand(bs * num_outputs, -1)
            .to(self.rank)
        )
    ...
    def beam_search(
        ...
        assert (
            how_many_outputs <= beam_size
        ), "requested output per sequence must be lower than beam width"
        assert (
            sample_or_max == "max" or sample_or_max == "sample"
        ), "argument must be chosen between 'max' and 'sample'"
        ...
        # init: ------------------------------------------------------------------
        init_dec_class = (
            torch.tensor([sos_idx] * bs).unsqueeze(1).type(torch.long).to(self.rank)
        )
        init_dec_logprob = (
            torch.tensor([0.0] * bs).unsqueeze(1).type(torch.float).to(self.rank)
        )
        log_probs = self.forward_dec(
            cross_input=cross_enc_output,
            enc_input_num_pads=enc_input_num_pads,
            dec_input=init_dec_class,
            dec_input_num_pads=[0] * bs,
        )
        if sample_or_max == "max":
            _, topi = torch.topk(log_probs, k=beam_size, sorted=True)
        else:  # sample
            topi = torch.exp(log_probs[:, 0, :]).multinomial(
                num_samples=beam_size, replacement=False
            )
        ...
        cross_enc_output = cross_enc_output.reshape(
            bs * beam_size, enc_seq_len, d_model
        ).contiguous()
        enc_input_num_pads = [
            enc_input_num_pads[i] for i in range(bs) for _ in range(beam_size)
        ]
        ...
        for time_step in range(2, max_seq_len):
            ...
            if sample_or_max == "max":
                _, topi = torch.topk(
                    log_probs[:, time_step - 1, :], k=beam_size, sorted=True
                )
            else:  # sample
                topi = torch.exp(log_probs[:, time_step - 1, :]).multinomial(
                    num_samples=beam_size, replacement=False
                )

            top_beam_size_word_classes = topi.reshape(bs, beam_size, beam_size)

            top_beam_size_word_logprobs = log_probs[:, time_step - 1, :].gather(
                dim=-1, index=topi
            )
            top_beam_size_word_logprobs = top_beam_size_word_logprobs.reshape(
            ...
            # are set to zero
            there_is_eos_mask = (
                (loop_dec_classes.view(bs, beam_size, time_step) == eos_idx)
            ...
            top_beam_size_word_logprobs[:, :, 1:].masked_fill_(
                there_is_eos_mask, -999.0
            )

            comparison_logprobs = loop_cumul_logprobs + top_beam_size_word_logprobs

            comparison_logprobs = comparison_logprobs.contiguous().view(
                bs, beam_size * beam_size
            )
            ...
            lastword_top_beam_size_classes = (
                which_sequence_top_beam_size_word_classes.gather(
                    dim=-1, index=which_word
                )
            )
            lastword_top_beam_size_logprobs = (
                which_sequence_top_beam_size_word_logprobs.gather(
                    dim=-1, index=which_word
                )
            )

            new_loop_dec_classes = torch.cat(
                (new_loop_dec_classes, lastword_top_beam_size_classes), dim=-1
            )
            new_loop_dec_logprobs = torch.cat(
                (new_loop_dec_logprobs, lastword_top_beam_size_logprobs), dim=-1
            )
            ...
            loop_num_elem_vector = loop_num_elem_vector.view(bs, beam_size)[
                [bs_idxes, which_sequence]
            ].view(bs * beam_size)
            there_was_eos_mask = (
                (loop_dec_classes[:, :, :-1].view(bs, beam_size, time_step) == eos_idx)
                .sum(dim=-1)
                .type(torch.bool)
                .view(bs * beam_size)
            )
            loop_num_elem_vector = loop_num_elem_vector + (
                1 * (1 - there_was_eos_mask.type(torch.int))
            )
        ...
        for i in range(bs):
            for j in range(how_many_outputs):
                ...
                res_caption_pred[i].append(
                    loop_dec_classes[
                        i, idx, : loop_num_elem_vector[i * beam_size + idx]
                    ].tolist()
                )
                res_caption_logprob[i].append(
                    loop_dec_logprobs[
                        i, idx, : loop_num_elem_vector[i * beam_size + idx]
                    ]
                )

        flatted_res_caption_logprob = [
            logprobs for i in range(bs) for logprobs in res_caption_logprob[i]
        ]
        flatted_res_caption_logprob = torch.nn.utils.rnn.pad_sequence(
            flatted_res_caption_logprob, batch_first=True
        )
