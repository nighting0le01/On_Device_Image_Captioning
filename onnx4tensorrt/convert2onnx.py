...
from onnx4tensorrt.End_ExpansionNet_v2_onnx_tensorrt import (
    create_pad_mask,
    create_no_peak_and_pad_mask,
)
from onnx4tensorrt.End_ExpansionNet_v2_onnx_tensorrt import (
    End_ExpansionNet_v2_ONNX_TensorRT,
    NUM_FEATURES,
    MAX_DECODE_STEPS,
)
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--vocab_path", type=str, default="./demo_material/demo_coco_tokens.pickle"
    )
    ...
    parser.add_argument(
        "--image_path_2", type=str, default="./demo_material/micheal.jpg"
    )
    parser.add_argument(
        "--load_model_path",
        type=str,
        default="./github_ignore_material/saves/rf_model.pth",
    )
    ...
    # Masks creation - - - - -
    batch_size = 1
    enc_mask = create_pad_mask(
        mask_size=(batch_size, sum(enc_exp_list), NUM_FEATURES),
        pad_row=[0],
        pad_column=[0],
    ).contiguous()
    no_peak_mask = create_no_peak_and_pad_mask(
        mask_size=(batch_size, MAX_DECODE_STEPS, MAX_DECODE_STEPS), num_pads=[0]
    ).contiguous()
    cross_mask = create_pad_mask(
        mask_size=(batch_size, MAX_DECODE_STEPS, NUM_FEATURES),
        pad_row=[0],
        pad_column=[0],
    ).contiguous()
    ...
    fw_dec_mask = (
        no_peak_mask.unsqueeze(2)
        .expand(batch_size, MAX_DECODE_STEPS, dec_exp, MAX_DECODE_STEPS)
        .contiguous()
        .view(batch_size, MAX_DECODE_STEPS * dec_exp, MAX_DECODE_STEPS)
    )

    bw_dec_mask = (
        no_peak_mask.unsqueeze(-1)
        .expand(batch_size, MAX_DECODE_STEPS, MAX_DECODE_STEPS, dec_exp)
        .contiguous()
    ...
    if args.onnx_simplify:
        ...
        try:
            ...
            print(
                "The simplification failed. In this case, we suggest to try the command line version."
            )
    ...
    if True:  # args.onnx_runtime_test:
        ...
        input_dict_1 = {
            ...
            "enc_mask": enc_mask.numpy(),
            "fw_dec_mask": fw_dec_mask.numpy(),
            "bw_dec_mask": bw_dec_mask.numpy(),
            "cross_mask": atten_mask.numpy(),
        }
        outputs_ort = ort_sess.run(None, input_dict_1)
        output_caption = tokens2description(
            outputs_ort[0][0].tolist(), coco_tokens["idx2word_list"], sos_idx, eos_idx
        )
    ...
    if args.onnx_tensorrt_test:
        ...
        engine = backend.prepare(
            onnx_model, device="CUDA:0", max_worker_size=args.max_worker_size
        )
        ...
        output_caption = tokens2description(
            output_data.tolist(), coco_tokens["idx2word_list"], sos_idx, eos_idx
        )
