...
def create_pad_mask(mask_size, pad_row, pad_column):
    ...
    pad_row_tens = torch.tensor([out_len]) - torch.tensor(pad_row).unsqueeze(-1).repeat(
        1, out_len
    )
    pad_col_tens = torch.tensor([in_len]) - torch.tensor(pad_column).unsqueeze(
        -1
    ).repeat(1, in_len)
    arange_on_columns = (
        torch.arange(in_len).unsqueeze(0).repeat(bs, 1) < pad_col_tens
    ).type(torch.int32)
    arange_on_rows = (
        torch.arange(out_len).unsqueeze(0).repeat(bs, 1) < pad_row_tens
    ).type(torch.int32)
...
def create_no_peak_and_pad_mask(mask_size, num_pads):
    ...
    triang_mask = (
        torch.tril(torch.ones(size=(seq_len, seq_len), dtype=torch.float), diagonal=0)
        .unsqueeze(0)
        .repeat(bs, 1, 1)
    )
...
class DecoderLayer_ONNX(nn.Module):
    ...
    def forward(
        self, x, n_indexes, cross_connection_x, fw_mask, bw_mask, cross_attention_mask
    ):
        # Pre-LayerNorm
        x2 = self.norm_1(x)
        x = x + self.dropout_1(
            self.dyn_exp(x=x2, n_indexes=n_indexes, fw_mask=fw_mask, bw_mask=bw_mask)
        )
...
class End_ExpansionNet_v2_ONNX_TensorRT(nn.Module):
    ...
    def __init__(
        ...
        self.encoders = nn.ModuleList(
            [
                EncoderLayer_ONNX(d_model, ff, num_exp_enc_list, drop_args.enc)
                for _ in range(N_enc)
            ]
        )
        self.decoders = nn.ModuleList(
            [
                DecoderLayer_ONNX(d_model, num_heads, ff, num_exp_dec, drop_args.dec)
    ...
    def single_step(
        ...
        time_step: int,
        x: torch.Tensor,
        loop_pred: torch.Tensor,
        loop_logprobs: torch.Tensor,
        fw_dec_mask: torch.Tensor,
        bw_dec_mask: torch.Tensor,
        cross_mask: torch.Tensor,
    ):
        log_probs = self.forward_dec(
            x, loop_pred[:, : time_step + 1], fw_dec_mask, bw_dec_mask, cross_mask
        )
    ...
    def forward(
        ...
        loop_pred, loop_logprobs = self.single_step(
            0,
            enc_x,
            loop_pred,
            loop_logprobs,
