...
if __name__ == "__main__":
    ...
    def build_engine_onnx(onnx_file_path, data_type):
        ...
        if data_type == "fp32":
            ...
            raise ValueError(
                "Unsupported type. Only the following types are supported: "
                + "fp32, fp16, int8."
            )
    ...
    if file_already_exist:
        print(
            "Engine File:"
            + str(args.engine_path)
            + " already exists, loading engine instead of ONNX file."
        )
    ...
    for binding in engine:
        ...
        print(
            ...
            + str(dtype)
            + " is input: "
            + str(engine.binding_is_input(binding))
        )
    ...
    output_caption = tokens2description(
        outputs[0]["host"].tolist(), coco_tokens["idx2word_list"], sos_idx, eos_idx
    )
