...
class ReinforceCiderReward:
    def __init__(self, training_references, eos_token, num_sampled_captions, rank):
        ...
        for i in range(len(training_references)):
            ...
            for caption in training_references[i]:
                ...
                caption = language_utils.add_space_between_non_alphanumeric_symbols(
                    caption
                )
    ...
    def compute_reward(
        ...
        all_images_ref_caption = [
            self.training_references[idx] for idx in all_images_idx
        ]
        all_images_ref_caption = list(
            itertools.chain.from_iterable(
                itertools.repeat(ref, self.num_sampled_captions)
                for ref in all_images_ref_caption
            )
        )

        cider_result = self.reinforce_cider.compute_score(
            hypo=all_images_pred_caption, refs=all_images_ref_caption
        )
        reward = (
            torch.tensor(cider_result[1])
            .to(self.rank)
            .view(batch_size, num_sampled_captions)
        )

        if all_images_base_caption is None:  # Mean base
            reward_base = (reward.sum(dim=-1, keepdim=True) - reward) / (
                num_sampled_captions - 1
            )
            ...
            base_cider_result = self.reinforce_cider.compute_score(
                hypo=all_images_base_caption, refs=all_images_ref_caption
            )
            reward_base = (
                torch.tensor(base_cider_result[1])
                .to(self.rank)
                .view(batch_size, num_sampled_captions)
            )
