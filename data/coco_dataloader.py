...
class CocoDataLoader(TransparentDataLoader):
    ...
    def __init__(
        ...
        if self.coco_dataset.use_images_instead_of_features:
            ...
            if self.coco_dataset.preproc_images_hdf5_filepath is not None:
                self.hdf5_img_file = h5py.File(
                    self.coco_dataset.preproc_images_hdf5_filepath, "r", rdcc_nbytes=0
                )
            ...
            self.train_preprocess_1 = torchvision.transforms.Compose(
                preprocess_layers_1
            )
            self.train_preprocess_2 = torchvision.transforms.Compose(
                preprocess_layers_2
            )
            ...
            self.hdf5_file = h5py.File(
                self.coco_dataset.precalc_features_hdf5_filepath, "r", rdcc_nbytes=0
            )
    ...
    def init_epoch(self, epoch_it, verbose=False):
        ...
        if self.dataloader_mode == "caption_wise":
            ...
            for img_idx in range(self.coco_dataset.train_num_images):
                num_captions = len(
                    self.coco_dataset.karpathy_train_list[img_idx]["captions"]
                )
            ...
            tailing_elements = len(img_idx_caption_id_pair_list) % (
                batch_size * self.num_procs
            )
            if tailing_elements != 0:
                img_idx_caption_id_pair_list = img_idx_caption_id_pair_list[
                    :-tailing_elements
                ]
            ...
            while i < len(img_idx_caption_id_pair_list):
                for idx_proc in range(self.num_procs):
                    ...
                    preprocessed_caption = self.preprocess(
                        self.coco_dataset.karpathy_train_list[img_idx]["captions"][
                            caption_id
                        ]
                    )
        ...
        if verbose:
            print(
                ...
                + "] "
                + __name__
                + ") Dataset epoch initialization "
                + str(time() - init_timer_start)
                + " s elapsed"
            )
            print(
                str(self.rank)
    ...
    def get_next_batch(
        self, verbose=False, get_also_image_idxes=False, get_also_image_path=False
    ):
        ...
        for i in range(len(img_idx_batch)):
            ...
            img_id_batch.append(
                self.coco_dataset.karpathy_train_list[img_idx]["img_id"]
            )

        if self.use_images_instead_of_features:
            batch_x, batch_x_num_pads = self.get_PADDED_image_batch_by_idx(
                img_idx_batch
            )
        else:
        ...
        if self.dataloader_mode == "caption_wise":
            batch_caption_y_as_string = copy.copy(
                self.caption_y[self.rank][self.batch_it[self.rank]]
            )
            batch_caption_y_encoded = language_utils.convert_allsentences_word2idx(
                batch_caption_y_as_string, self.coco_dataset.caption_word2idx_dict
            )
            batch_y, batch_y_num_pads = self.add_pad_according_to_batch(
                batch_caption_y_encoded, self.coco_dataset.caption_word2idx_dict["PAD"]
            )
            batch_y = torch.tensor(batch_y)
        else:  # image_wise
            batch_y = [
                self.coco_dataset.karpathy_train_list[img_idx]["captions"]
                for img_idx in img_idx_batch
            ]
        ...
        if verbose:
            if not self.use_images_instead_of_features:
                mean_src_len = int(
                    sum(
                        [
                            (len(batch_x[i]) - batch_x_num_pads[i])
            ...
            if self.dataloader_mode == "caption_wise":
                ...
                mean_trg_len = sum(
                    [(len(cap.split(" "))) for captions in batch_y for cap in captions]
                ) // sum([len(captions) for captions in batch_y])
        ...
        if get_also_image_path:
            for i in range(len(img_idx_batch)):
                idx = img_idx_batch[i]
                file_path_batch_x.append(
                    self.coco_dataset.karpathy_train_list[idx]["img_path"]
                )
    ...
    def get_batch_samples(self, dataset_split, img_idx_batch_list):
        ...
        for i in range(len(img_idx_batch_list)):
            ...
            if dataset_split == CocoDatasetKarpathy.TestSet_ID:
                caption_id = random.randint(
                    a=0,
                    b=len(self.coco_dataset.karpathy_test_list[img_idx]["captions"])
                    - 1,
                )
                caption = self.coco_dataset.karpathy_test_list[img_idx]["captions"][
                    caption_id
                ]
            elif dataset_split == CocoDatasetKarpathy.ValidationSet_ID:
                caption_id = random.randint(
                    a=0,
                    b=len(self.coco_dataset.karpathy_val_list[img_idx]["captions"]) - 1,
                )
                caption = self.coco_dataset.karpathy_val_list[img_idx]["captions"][
                    caption_id
                ]
            else:
                caption_id = random.randint(
                    a=0,
                    b=len(self.coco_dataset.karpathy_train_list[img_idx]["captions"])
                    - 1,
                )
                caption = self.coco_dataset.karpathy_train_list[img_idx]["captions"][
                    caption_id
                ]
            ...
            if dataset_split == CocoDatasetKarpathy.TestSet_ID:
                ...
                img_id_batch.append(
                    self.coco_dataset.karpathy_test_list[img_idx]["img_id"]
                )
            elif dataset_split == CocoDatasetKarpathy.ValidationSet_ID:
                batch_captions_y_as_string.append(preprocessed_caption)
                img_id_batch.append(
                    self.coco_dataset.karpathy_val_list[img_idx]["img_id"]
                )
            else:
                batch_captions_y_as_string.append(preprocessed_caption)
                img_id_batch.append(
        ...
        batch_caption_y_encoded = language_utils.convert_allsentences_word2idx(
            batch_captions_y_as_string, self.coco_dataset.caption_word2idx_dict
        )
        batch_y, batch_y_num_pads = self.add_pad_according_to_batch(
            batch_caption_y_encoded, self.coco_dataset.get_pad_token_idx()
        )
        batch_y = torch.tensor(batch_y)
        ...
        for img_idx in img_idx_list:
            if self.coco_dataset.preproc_images_hdf5_filepath is not None:
                ...
                list_of_images.append(
                    torch.tensor(np_array, dtype=torch.float32).unsqueeze(0)
                )
            else:
    ...
    def get_PADDED_bboxes_batch_by_id(self, img_id_list, verbose=False):
        ...
        if sum(list_of_num_pads) != 0:
            padded_batch_of_bboxes_tensor = torch.nn.utils.rnn.pad_sequence(
                list_of_bboxes_tensor, batch_first=True
            )
    ...
    def get_images_by_idx(
        self, img_idx, dataset_split, transf_mode="train", get_also_id=False
    ):
        if dataset_split == CocoDatasetKarpathy.TestSet_ID:
    ...
    def preprocess(self, caption):
        ...
        caption = (
            [self.coco_dataset.get_sos_token_str()]
            + language_utils.tokenize(caption)[0]
            + [self.coco_dataset.get_eos_token_str()]
        )
        preprocessed_tokenized_caption = []
        for word in caption:
            if word not in self.coco_dataset.caption_word2idx_dict.keys():
                preprocessed_tokenized_caption.append(
                    self.coco_dataset.get_unk_token_str()
                )
            else:
